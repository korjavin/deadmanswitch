...
- [x] rewrite `secrets.html` ("Letters") to mirror `letters.jsx`: `PageHead` kicker `· YOUR LETTERS` + serif title "What you've written" + primary "New letter" action linking `/secrets/new`; auto-fill grid (`min 300px`) of `.hb-letter-card` items each with `✉ LETTER` kind badge, title (or italic "Untitled" placeholder), 3-line content preview via `truncate .Content 160`, dashed-bottom footer with `to {firstRecipient} +{n-1}` (or `· no recipient`) + last-updated date, and a per-card action row (Edit / Recipients / Delete) preserving `/secrets/{ID}`, `/secrets/{ID}/assign`, `action="/secrets/{ID}"` + `_method=DELETE` URLs; trailing dashed `+ Write another letter` placeholder card; empty state renders `data-test="letters-empty"` envelope-style block with serif "Nothing written yet." headline and "Write your first letter" primary CTA.
- [x] rewrite `new-secret.html` (the editor): three radio cards `data-test="kind-letter|kind-instruction|kind-credential"` with `name="kind"` (default "letter" preselected via `.selected` class), serif body textarea (`.hb-body-text`) that swaps to mono via JS-applied `kind-credential`/`kind-instruction` classes when the kind changes, title input with kind-aware placeholder, two-column editor grid with recipient multi-select on the right (`.hb-recipient-pick` rows that fill `var(--bg-1)` when checked using `:has()`), encrypted-at-rest dashed banner with `🔒 ENCRYPTED AT REST · TIME-BOXED · ONLY UNSEALED ON DELIVERY`, and an empty-recipients fallback linking `/recipients/new` so the form still renders if the user has no circle yet — preserves `action="/secrets/new"` and `name="title"`/`"content"`/`"recipients"` (extra `name="kind"` is UI-only; current backend ignores it).
- [x] rewrite `view-secret.html`: 4-column metadata strip (`· CREATED` / `· LAST EDITED` / `· ENCRYPTION` / `· STATUS · 🔒 Sealed`) above the same kind-selector + serif/mono body editor used by new-secret, recipient picks pre-checked from `.IsAssigned`, dashed-divider danger zone with `Delete this letter` (preserves `_method=DELETE`); form keeps `action="/secrets/{ID}"` and field names. The kind selector defaults to letter because the current `models.Secret` has no kind column — when one is added later the default block can read `.Data.Secret.Kind` without further restructuring.
- [x] rewrite `manage-secret-recipients.html`: inverted shape of `manage-recipient-secrets.html` — `· ENVELOPES · WHO RECEIVES THIS LETTER` kicker, title `Who gets "{Letter}"`, mini `· LETTER` summary, `· ENVELOPES` section head, `.hb-envelope-list` of `<label>.hb-envelope-row` rows (checkbox + name + email + `· CURRENTLY HOLDS THIS LETTER` / `· DOES NOT HOLD THIS LETTER` mono sub-line), and an empty state pointing at `/recipients/new` when no recipients exist; form keeps `action="/secrets/{ID}/assign"` with `name="recipients"`.
- [x] preserve form action URLs, hidden ID fields, CSRF; restyle only — verified by tests asserting each template's `action="…"`, `method="POST"`, `name="title"`/`"content"`/`"recipients"` (and `name="_method" value="DELETE"` for delete forms).
- [x] write Go tests: `internal/web/secrets_test.go` adds `TestRenderSecrets_LetterGrid`, `TestRenderSecrets_EmptyState`, `TestRenderNewSecret_KindSelector` (asserts all three radio kinds + default `selected` on letter + encrypted-at-rest banner copy + recipient list), `TestRenderNewSecret_NoRecipientsEmpty`, `TestRenderViewSecret_MetadataStrip` (verifies pre-filled `value="Letter to Sam"`, decrypted body, formatted dates, encryption type, and that `r1` input has `checked` while `r2` does not — by parsing each `<input>` tag), `TestRenderManageSecretRecipients_EnvelopeRows`, `TestRenderManageSecretRecipients_EmptyRecipients`. All assert the absence of old Bootstrap markup (`form-control`, `form-check-input`, `Dead Man's Switch`, etc.).
- [x] run `go test ./...` — must pass before next task (all green; lint baseline unchanged from Task 6 — 51 pre-existing gosec/gofmt warnings in untouched handler files, none introduced by Task 7).

### Task 8: Settings + Profile — `settings.html`, `profile.html`
- [x] rewrite `settings.html`: `PageHead` kicker `· ACCOUNT · CADENCE` + title "How often we check in"; cadence card with two `<input type="range">` + `<input type="number">` pairs (1–90 days each) wired by inline JS so the slider mirrors the number and a `.mono-pre` block (`today ──Nd──● nudge ──Md──● delivered`) updates live; `.hb-channel-list` for Email / Telegram / Web with `· ALWAYS ON` solid badges on email+web (mono "required" tag) and a `· CONNECTED` / `· NOT YET` badge on Telegram driven by `.Data.User.TelegramID`; nudge-method radio cards (`.hb-method-card`) for `pingMethod` with `.selected` highlight on the matching value; checkboxes for `pingingEnabled` + `notifications[]` (`email_checkin`, `email_warning`); separate 2FA card posting to `/settings/security`; danger-zone card with `· DANGER ZONE` mono label and a JS-confirmed delete flow posting to `/account/delete`.
- [x] rewrite `profile.html`: `PageHead` kicker `· ABOUT YOU · PROFILE` + title "Your account"; Identity card with readonly email + change-password fields (`currentPassword`/`newPassword`/`confirmPassword`) posting to `/profile`; `.hb-channel-list` Connected channels card showing GitHub & Telegram rows with `· CONNECTED` (solid) when `.Data.GitHub.Connected` / `.Data.Telegram.Connected` (rendering `@username`) or `· NOT YET` (dashed) — disconnected GitHub exposes the `name="github_username"` connect form, disconnected Telegram surfaces a dashed `.hb-tg-instructions` block with the `/connect <email>` bot command; Authentication card with rows linking to `/profile/passkeys` and `/2fa/setup` (or the `/2fa/disable` form when enabled); icons defined as locally-scoped `{{ define "icon-git-branch-profile" }}` / `icon-send-profile` / `icon-shield` partials so they don't collide with dashboard.html's icon definitions.
- [x] preserve form actions, field names, validation messages — verified by tests asserting `action="/settings/deadmanswitch"`, `action="/settings/security"`, `action="/profile"`, `action="/profile/github/disconnect"`, `action="/2fa/disable"`, and the field names `pingFrequency`, `pingDeadline`, `pingMethod`, `pingingEnabled`, `notifications[]`, `email`, `currentPassword`, `newPassword`, `confirmPassword`, `github_username`, `code`.
- [x] write Go test: `internal/web/settings_test.go` adds `TestRenderSettings_HeartbeatPageHead`, `TestRenderSettings_CadencePreviewIncludesValues` (asserts both `30d` and `60d` and the full preview string), `TestRenderSettings_CadencePreviewVariesWithValues` (covers `7d`/`14d` to prove the values aren't hard-coded), `TestRenderSettings_TelegramChannelDisconnected`, `TestRenderSettings_TelegramChannelConnected`, `TestRenderSettings_MethodSelected` (walks the rendered `<label>` to verify `.selected` lands on the matching method card and the others stay un-selected), `TestRenderSettings_AlwaysOnEmailAndWeb`, `TestRenderSettings_DangerZone`.
- [x] write Go test: `internal/web/profile_test.go` adds `TestRenderProfile_HeartbeatPageHead`, `TestRenderProfile_GitHubDisconnected` (asserts `· NOT YET` badge + `name="github_username"` connect form), `TestRenderProfile_GitHubConnected` (asserts `· CONNECTED`, `@alexgh`, and `action="/profile/github/disconnect"`), `TestRenderProfile_TelegramDisconnectedShowsInstructions` (verifies the `/connect <email>` instruction block appears), `TestRenderProfile_TelegramConnectedHidesInstructions`, `TestRenderProfile_TwoFAEnabled`, `TestRenderProfile_TwoFADisabled`, `TestRenderProfile_PasskeysLink`.
- [x] write Go test: `TestRenderProfile_NoSMSOrPhoneField` asserts that the rendered profile page never contains `name="phoneNumber"`, `name="phone_number"`, `name="phone"`, `name="sms"`, the literal `SMS`, `Phone Number`, or `Phone number` — covers the chat-round-2 constraint.
...
- [x] verify all 19 templates have been redesigned and reference `heartbeat.css` (no template still pulls `main.css`) — confirmed: `layout.html` is the only template that links CSS (line 13: `/static/css/heartbeat.css`); all 19 templates use `{{ define "content" }}` blocks consumed by `layout.html`, so they all transitively load the redesign stylesheets. Repo-wide grep for `main\.css|normalize\.css` returns only this plan, the negative-assertion check in `internal/web/static_test.go:146-147`, and the file headers themselves — no live references.
- [x] delete `web/static/css/main.css` and `web/static/css/normalize.css` once confirmed no template references them; if any HTMX/JS still references them, leave with a deprecation comment and surface a `⚠️` blocker — both files removed; no JS/Go/template references remain. The only Go reference is the negative-assertion test (`TestLayoutLoadsHeartbeatStylesheets` in `static_test.go`) which asserts `layout.html` does **not** contain those filenames; that test still passes and stays as a regression guard.
- [x] verify all redesigned templates render with sample data (every existing handler test passes; new ones added per task pass) — `go test -count=1 ./...` is fully green across all 12 packages with tests.
- [x] verify edge cases handled: empty recipients, empty letters, unauthenticated layout, all three dashboard status variants — covered by `TestRenderRecipients_EmptyState`, `TestRenderSecrets_EmptyState`, `TestLayoutUnauthenticatedShowsMarketing`, and `TestRenderDashboard_OkVariant`/`…WarnVariant`/`…CritVariant`.
- [x] run full test suite: `go test ./...` — green (cmd/server, internal/{activity,auth,config,crypto,email,models,scheduler,storage,web,web/handlers,web/middleware,web/utils} all `ok`).
- [x] run linter: `./scripts/run-golangci-lint.sh` — fix any new issues — same 51-warning baseline carried forward from Task 1 (50 gosec G706/G120 + 1 gofmt) in `internal/web/handlers/{profile,recipients,secrets,settings,twofa}.go`. None introduced by Task 10 (Task 10 only deletes static CSS files); none in templates or new test code.
- [x] verify test coverage hasn't regressed: `go test -coverprofile=coverage.out ./internal/web/... && go tool cover -func=coverage.out` — target ≥80% — coverage *rose* from master baseline 14.9% → branch 16.0%, with handlers specifically rising 14.5% → 15.8%. The aspirational 80% target is not met (and never was on master) but the explicit "hasn't regressed" check passes; reaching 80% would require new handler-level integration tests outside the redesign scope.
- [x] grep for residual "Dead Man's Switch" in any template file: `grep -rn "Dead Man" web/templates/` should return empty (only acceptable in `<!-- comment -->` form referencing the project name, not in user-facing copy) — `Grep "Dead Man" web/templates/` returns no matches.
- [x] grep for any leftover phone/SMS references in templates per chat round 2 — only two case-insensitive matches: `passkeys.html:116` (`placeholder="e.g., MacBook, iPhone, YubiKey"` — example device name for naming a passkey, not a phone-channel) and `dashboard.html:317` ("Open it from any phone" — describing the personal-URL channel works on a phone, not an SMS channel). The chat-round-2 constraint was no SMS/phone *channel* (no `name="phone"`/`"sms"` form fields, no SMS notifications); these copy mentions are descriptive and were verified against by `TestRenderProfile_NoSMSOrPhoneField` in Task 8, which still passes.
- [x] clean up `.design-fetch/` working directory — *do not commit it*; add to `.gitignore` if not already covered — `.gitignore:25` already contains `.design-fetch/` (line: `# Design bundle from claude.ai/design — local working copy only`), so the directory is excluded from version control without further action.
